...
export default function Pricing() {
  ...
  return <Layout>
    ...
    /* Header */
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <p className="text-xl text-muted-foreground">
            One career at a time. Deep, honest guidance — not a generic subscription.
          </p>
        </div>
              ...
              <div className="flex justify-between items-start mb-6">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-xs font-semibold text-primary uppercase tracking-widest">Growvia Starter</span>
                  </div>
                  <h2 className="text-2xl font-bold text-white">One Career, Full Clarity</h2>
                  <p className="text-sm text-muted-foreground mt-1">Choose any 1 career — get everything for it.</p>
                </div>
    ...
    /* Features */
              ...
              <ul className="space-y-3.5 mb-8">
                {pricingFeatures.map((feature, i) => <li key={i} className="flex items-start gap-3">
    ...
    return <button
      ...
      className={`flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl border text-left text-sm font-medium transition-all ${selectedCareer === career.id ? "border-primary bg-primary/10 text-white" : "border-white/10 bg-white/[0.03] text-muted-foreground hover:border-white/25 hover:text-white"}`}
    >
                        <Icon className={`w-4 h-4 flex-shrink-0 ${selectedCareer === career.id ? "text-primary" : "text-muted-foreground"}`} />
                        <span className="truncate">{career.title}</span>
                        {selectedCareer === career.id && <Check className="w-3.5 h-3.5 text-primary ml-auto flex-shrink-0" />}
                      </button>;
  })}
                </div>
                {!selectedCareer && <p className="text-xs text-muted-foreground mt-2.5">
    ...
    /* CTA */
              ...
              <Button
    asChild={!!selectedCareer}
    size="lg"
    disabled={!selectedCareer}
    className={`w-full font-bold h-14 text-lg flex items-center gap-2 transition-all rounded-2xl ${selectedCareer ? "bg-primary text-primary-foreground hover:opacity-90" : "bg-white/10 text-white/40 cursor-not-allowed"}`}
  >
                {selectedCareer ? <Link href="/login">
                    Unlock {careers.find((c) => c.id === selectedCareer)?.title} Roadmap — ₹99
                    <ChevronRight className="w-5 h-5" />
                  </Link> : <span>Select a Career to Continue</span>}
              </Button>
    ...
    { label: "All 8 Careers", value: "\u20B9792", note: "Explore everything" }
  ...
  ].map((faq, i) => <div key={i} className="border-b border-white/10 pb-6">
